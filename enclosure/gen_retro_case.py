...
scad_code = '''// Retro PC Style - Desktop Monitor Enclosure for ESP32
...
# Write SCAD
scad_path = r'E:\CODEX-Pj\ESP32\enclosure\retro_pc_case_v2.scad'
stl_path = r'E:\CODEX-Pj\ESP32\enclosure\retro_pc_case_v2.stl'

with open(scad_path, 'w', encoding='utf-8') as f:
    f.write(scad_code)
print('SCAD written')

# Remove any BOM and fix 
with open(scad_path, 'rb') as f:
    data = f.read()
if data.startswith(b'\\xef\\xbb\\xbf'):
    data = data[3:]
with open(scad_path, 'wb') as f:
    f.write(data)

# Fix  - add it after the header
content = open(scad_path, 'r', encoding='utf-8').read()
content = content.replace('// ===== PARAMETERS', '\\ = 60;\\n// ===== PARAMETERS')
with open(scad_path, 'w', encoding='utf-8') as f:
    f.write(content)
...
# Render
oscad = os.path.expandvars(r'%TEMP%\\oscad\\openscad-2021.01\\openscad.exe')
result = subprocess.run([oscad, '-o', stl_path, scad_path], capture_output=True, text=True, timeout=120)
if os.path.exists(stl_path):
    sz = os.path.getsize(stl_path)
    print(f'STL: {sz} bytes')
else:
    print('FAILED')
